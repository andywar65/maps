...
async function load_borders() {
  ...
  if (zoom<10){
    border_url = `/maps/api/borders/lo/?in_bbox=${map.getBounds().toBBoxString()}`
  } else if (zoom<14){
    border_url = `/maps/api/borders/md/?in_bbox=${map.getBounds().toBBoxString()}`
  } else {
    border_url = `/maps/api/borders/hi/?in_bbox=${map.getBounds().toBBoxString()}`
  }
